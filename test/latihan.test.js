...
describe('Greeting Component', () => {
    test("Displays greeting with user's name", () => {
      render(<Greeting name="adam" />);
      screen.debug(); 
      const greetingMessage = screen.getByTestId('greeting-message');
      expect(greetingMessage).toHaveTextContent('Hello, adam');
    });
  
    test("Displays greeting with instructor's name", () => {
      render(<Greeting name="adamzalfa" />);
      screen.debug();
      const greetingMessage = screen.getByTestId('greeting-message');
      expect(greetingMessage).toHaveTextContent('Hello, adamzalfa');
    });
  });
